...
public interface BillRepository extends JpaRepository<BillMaster, Long> {
    ...
    Optional<BillMaster> getBillDetailById(@Param("billId") Long billId);
     ...
     */
    @Query("SELECT NEW com.ap_express_server.models.bill.BillDto(billMst.id, billMst.billNo, billMst.vendorId, ven.name, DATE(billMst.billDate), billMst.billAmount, billMst.createdBy, billMst.createdDate, billMst.status, " +
            "CASE billMst.status " +
            "    WHEN 'P' THEN 'Pending' " +
     ...
     */
    @Query("select new com.ap_express_server.models.bill.BillDto(bill.paymentAmount, bill.billBalance) from BillMaster bill")
    Optional<BillDto> getBillBalance(@Param("billId") Long billId);
    ...
    List<DropDownDto> getVendorRelatedActiveBillList(@Param("vendorId") Long vendorId);

    /**
     * Retrieves set of ApprovedBillChartData objects data.
     * @return list of data in a ApprovedBillChartData format
     */
    @Query("SELECT MONTH(bill.billDate) AS month, COUNT(bill) AS noOfBills FROM BillMaster bill where bill.status = 'A' GROUP BY MONTH(bill.billDate)")
